...
solar_winkel <- read_csv('../data/sonnenwinkel_20190419.csv', skip = 3) %>%
  ...
  mutate('kartesisch' = transpose(polar_zu_kartesisch(azimuth = azimuth_rad, elevation = elevation_rad))) #%>%
#  unnest(kartesisch) %>%
#  group_by(datetime) %>% 
...
#  filter(!is.na(uhrzeit)) %>%
  ...
  mutate('flache_proj_flaeche' = mapply(skalarprodukt, kartesisch, flache_position)) %>%
  select("MESS_DATUM", "direkte_strahlung", "kartesisch", "flache_proj_flaeche")
```

```{r fester Kippwinkel TEST, eval=FALSE, include=FALSE}
position_vec5 <- polar_zu_kartesisch(grad_zu_rad(180), grad_zu_rad(95))

solar_data_karfreitag <- solar_data_karfreitag %>%
#  filter(!is.na(uhrzeit)) %>%
  mutate("fuenf_grad" = transpose(position_vec5)) %>%
  mutate('fuenf_grad_flaeche' = mapply(skalarprodukt, kartesisch, fuenf_grad)) %>%
  select(-"fuenf_grad")

position_vec10 <- polar_zu_kartesisch(grad_zu_rad(180), grad_zu_rad(90 + 10))

solar_data_karfreitag <- solar_data_karfreitag %>%
#  filter(!is.na(uhrzeit)) %>%
  mutate('zehn_grad_flaeche' = mapply(skalarprodukt, kartesisch, transpose(position_vec10)))

solar_data_karfreitag <- solar_data_karfreitag %>%
#  filter(!is.na(uhrzeit)) %>%
  mutate("zehn_grad" = transpose(position_vec10)) %>%
  mutate('zehn_grad_flaeche2' = mapply(skalarprodukt, kartesisch, zehn_grad)) %>%
  select(-"zehn_grad")
...
#  filter(!is.na(uhrzeit)) %>%
 # mutate("zehn_grad" = transpose(position_vec10)) %>%
  mutate('fuenfzehn_grad_flaeche' = mapply(skalarprodukt, kartesisch, berechne_panelwinkel_in_skalar(15)))
 # select(-"zehn_grad")

position_vec20 <- polar_zu_kartesisch(grad_zu_rad(180), grad_zu_rad(110))


solar_data_karfreitag <- solar_data_karfreitag %>%
#  filter(!is.na(uhrzeit)) %>%
  mutate("zwanzig_grad" = transpose(position_vec20)) %>%
  mutate('zwanzig_grad_flaeche' = mapply(skalarprodukt, kartesisch, zwanzig_grad)) %>%
  select(-"zwanzig_grad")
...
#lapply(berechne_panelwinkel_in_skalar(0), function(x) -x)

solar_data_karfreitag <- solar_data_karfreitag %>%
  mutate('fuenfzehn_grad_flaeche' = mapply(skalarprodukt, kartesisch, berechne_panelwinkel_in_skalar(15))) %>%
  mutate('dreißig_grad_flaeche' = mapply(skalarprodukt, kartesisch, berechne_panelwinkel_in_skalar(30))) %>%
  mutate('fuenfundvierzig_grad_flaeche' = mapply(skalarprodukt, kartesisch, berechne_panelwinkel_in_skalar(45))) %>%
  mutate('sechzig_grad_flaeche' = mapply(skalarprodukt, kartesisch, berechne_panelwinkel_in_skalar(60)))
```
...
mapply(skalarprodukt, solar_data_karfreitag$kartesisch, berechne_panelwinkel_in_skalar(0))
