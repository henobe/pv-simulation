...
```{r Summierung}
helper_optim <- function(x){
  helper_summe(x[1], x[2])
}

optimised_angles <- optim(c(0,30), helper_optim, control = list(fnscale = -1))

solar_data_karfreitag <- solar_data_karfreitag %>%
  mutate('strahlung_beste_kippung' = direkte_strahlung*berechne_relative_einstrahlflaeche(kartesisch, berechne_normalenvektor_panel(drehwinkel = optimised_angles$par[1], kippwinkel = optimised_angles$par[2], radiant = FALSE)))

ggplot(solar_data_karfreitag, aes(x = MESS_DATUM)) +
  geom_line(aes(y = direkte_strahlung, colour = "Strahlung Flach")) +
  geom_line(aes(y = strahlung_perfekte_nachfuehrung, colour = "Strahlung nachgeführt")) +
  geom_line(aes(y = strahlung_beste_kippung, colour = "Strahlung optimal gekippt")) +
  geom_label(aes(x = solar_data_karfreitag$MESS_DATUM[65], y = 35, label = paste("Winkel: Azim. ", round(optimised_angles$par[1], digits = 2), " Elev. ", round(optimised_angles$par[2], digits = 2)))) +
  #scale_colour_manual(values = c("blue", "red")) +
  labs(
    y = "Strahlung in J/cm^2",
    x = "Uhrzeit",
